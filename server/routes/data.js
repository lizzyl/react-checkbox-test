	...
	let promise = new Promise((resolve, reject) => {
		fs.readFile("./public/database/database.json", "utf-8", (err, data) => {
			if (err) {
				...
				dataBase = data;
				console.log('database: ' + data.recruitment.departments)
				recruitmentNames = data.recruitment.departments;
				resolve();
			}
		})
	});
...
exports.recruitment = (req, res) => {
	...
	if (recruitmentNames) {
		sendData.status = 1;
		sendData.msg = "success";
		sendData.title = dataBase.recruitment.title;
		sendData.departments = recruitmentNames;
		console.log('type1: ' + recruitmentNames[0].title)
	} else {
		sendData.msg = "error";
	}
